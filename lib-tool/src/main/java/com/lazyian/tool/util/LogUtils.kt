...
import com.lazyian.tool.ext.ext.okHttpLog
...
object LogUtils {
    private const val DEFAULT_TAG = "OKHttpLog"
    fun debugInfo(tag: String?, msg: String?) {
        if (!okHttpLog || TextUtils.isEmpty(msg)) {
            return
        }
        if (msg != null) {
            Log.d(tag, msg)
     ...
     * @param msg 日志内容
     */
    fun debugLongInfo(tag: String?, msg1: String) {
        var msg = msg1
        if (!okHttpLog || TextUtils.isEmpty(msg)) {
            return
        }
