...
object ValidatorUtil {
     ...
     * 正则表达式：验证用户名
     */
    private const val REGEX_USERNAME = "^[a-zA-Z]\\w{5,17}$"
    ...
    private const val REGEX_EMAIL =
        "\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*" //"^([a-z0-9A-Z]+[-|\\.]?)+[a-z0-9A-Z]@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)+[a-zA-Z]{2,}$";

    /**
     * 正则表达式：验证汉字
     */
    private const val REGEX_CHINESE = "^[\u4e00-\u9fa5]{2,10},*$"
     ...
     * 正则表达式：验证IP地址
     */
    private const val REGEX_IP_ADDR = "(25[0-5]|2[0-4]\\d|[0-1]\\d{2}|[1-9]?\\d)"
    ...
    // 金额验证
    fun isPrice(str: String): Boolean {
        val pattern = Pattern
    ...
    /**
     * isVcodeEqual(验证验证码是否与服务器发送来的一致)<br></br>
     *
     * @param vcodeStr
     * @return boolean
     * @throws
     * @since 1.0.0
     */
    fun isVcodeEqual(vcodeStr: String, netVcode: String): Boolean {
     ...
     */
    fun validateIDStr(IDStr: String): String {
        ...
        val strVerifyCode = ValCodeArr[modValue].toUpperCase()
        Ai = Ai + strVerifyCode
        if (IDStr.length == 18) {
            if (Ai != IDStr) {
